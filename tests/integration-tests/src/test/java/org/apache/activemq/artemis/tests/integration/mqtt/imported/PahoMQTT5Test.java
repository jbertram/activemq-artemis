...
public class PahoMQTT5Test extends MQTT5TestSupport {

   private static final Logger log = Logger.getLogger(PahoMQTT5Test.class);
...
//         {WS}
   ...
   public PahoMQTT5Test(String protocol) {
   ...
   /*
    * This is good for testing external clients against a broker you can debug
    */
   @Ignore
    ...
    *
    * Publish the will message immediately after disconnect when there is no session expiry or will delay intervals.
    */
   @Test(timeout = DEFAULT_TIMEOUT)
   public void testWillFlagWithNoSessionExpiryDelayAndNoWillDelay() throws Exception {
    ...
    * ClientID is opened before the Will Delay Interval has elapsed.
    *
    * Publish the will message after disconnect when the will delay interval elapses before the session expiry interval.
    */
   @Test(timeout = DEFAULT_TIMEOUT)
   ...
   public void testEmptyClientIDWithoutCleanStart() throws Exception {
      ...
      try {
         ...
         assertEquals(Byte.toUnsignedInt((byte) 0x85), e.getReasonCode());
      }
   }
   ...
   private interface DefaultMqttCallback extends MqttCallback {
      ...
      @Override
      default void messageArrived(String topic, MqttMessage message) throws Exception {
      }
   ...
   private class LatchedMqttCallback implements DefaultMqttCallback {
      CountDownLatch latch;

      LatchedMqttCallback(CountDownLatch latch) {
         this.latch = latch;
      }
