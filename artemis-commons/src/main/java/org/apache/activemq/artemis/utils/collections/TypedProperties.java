...
public class TypedProperties {
      ...
      if (properties == null) {
         ...
         for (Entry<SimpleString, PropertyValue> entry : properties.entrySet()) {
            ...
            if (val instanceof SimpleString string) {
               m.put(entry.getKey().toString(), string.toString());
            } else {
   ...
   public static void setObjectProperty(final SimpleString key, final Object value, final TypedProperties properties) {
      if (value == null) {
         ...
         properties.putBooleanProperty(key, boolean1);
      } else if (value instanceof Byte byte1) {
         properties.putByteProperty(key, byte1);
      } else if (value instanceof Character character) {
         properties.putCharProperty(key, character);
      } else if (value instanceof Short short1) {
         properties.putShortProperty(key, short1);
      } else if (value instanceof Integer integer) {
         properties.putIntProperty(key, integer);
      } else if (value instanceof Long long1) {
         properties.putLongProperty(key, long1);
      } else if (value instanceof Float float1) {
         properties.putFloatProperty(key, float1);
      } else if (value instanceof Double double1) {
         properties.putDoubleProperty(key, double1);
      } else if (value instanceof String string) {
         properties.putSimpleStringProperty(key, SimpleString.of(string));
      } else if (value instanceof SimpleString string) {
         properties.putSimpleStringProperty(key, string);
      } else if (value instanceof byte[] bytes) {
