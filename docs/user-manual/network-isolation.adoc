...
*Native* quorum voting requires the participation of the other _active_ brokers in the cluster.
Of course this requires that there are, in fact, other active brokers in the cluster which means native quorum voting won't work with a single HA pair of brokers.
...
The broker ships with a reference plugin implementation based on https://zookeeper.apache.org/[Apache ZooKeeper] - a common implementation used for this kind of use-case.

The main benefit of pluggable quorum voting is that is releases the broker from the responsibility of establishing a reliable vote.
