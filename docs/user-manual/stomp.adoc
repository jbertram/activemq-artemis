...
See the general xref:protocols-interoperability.adoc[Protocols and Interoperability] chapter for details on configuring an `acceptor` for STOMP.
...
STOMP connections therefore default to a `connection-ttl` value of 1 minute (see chapter on xref:connection-ttl.adoc[connection-ttl] for more information.
...
The filter expression syntax follows the _core filter syntax_ described in the xref:filter-expressions.adoc[Filter Expressions] documentation.
...
The default value of `stompMinLargeMessageSize` is the same as the default value of xref:large-messages.adoc#configuring-parameters[minLargeMessageSize].
...
This is broadly discussed in the xref:flow-control.adoc[Flow Control] chapter.
