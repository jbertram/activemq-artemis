...
Because the name is a UUID it is impossible to create an `address-setting` or `security-setting` for it whose `match` is anything but `#`.
...
When the `temporary-queue-namespace` is set and a temporary queue is created with a UUID name then the broker will prepend the `temporary-queue-namespace` value along with the `delimiter` value configured in `wildcard-addresses` (defaults to `.`) to both the address and queue name.
...
Using this configuration any temporary UUID queue will have metrics disabled.

[NOTE]
====
A UUID is a 36-character string that follows the pattern `XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX` where:
