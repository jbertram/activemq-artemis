...
Diverts can also be configured to apply a xref:transformers.adoc[`Transformer`].
...
See the documentation on xref:using-server.adoc[adding runtime dependencies] to  understand how to make your transformer available to the broker.
...
Diverted messages get xref:copied-message-properties.adoc[special properties].
